...
/**
 * Services and locations are the only facts that become keywords, so they must
...
export async function loadSeoBrain(supabase: Client, businessId: string): Promise<BrainSeoContext> {
  ...
  return {
    ...
    services: bucket(SERVICE_HINT, ["offers"]).filter(nameLike),
    locations: bucket(LOCATION_HINT).filter(nameLike),
